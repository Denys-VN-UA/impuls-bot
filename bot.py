...
# PRO-адаптация (можно включать/выключать через Railway Variables)
QUALITY_MODE = int(os.getenv("QUALITY_MODE", "0"))  # 1 = строгие фильтры (меньше сигналов, выше качество)
ADAPTIVE_PRO_MAX = int(os.getenv("ADAPTIVE_PRO_MAX", "0"))  # 1 = авто-адаптация порогов под рынок
DIVERSIFY_WINDOW_MINUTES = int(os.getenv("DIVERSIFY_WINDOW_MINUTES", "25"))  # не спамить одну пару
DIVERSIFY_PENALTY_POINTS = float(os.getenv("DIVERSIFY_PENALTY_POINTS", "5"))  # штраф к вероятности (в п.п.)
...
def adaptive_thresholds(atr_pct: float, chop: float, impulse: float) -> Tuple[int, int, Tuple[float, float], Tuple[float, float]]:
    """Возвращает (min_prob, hybrid_min_prob, rsi_band_call, rsi_band_put).
    Идея: в «пиле/флете» требуем выше вероятность и более строгий RSI,
    в «импульсе/тренде» можем чуть смягчить порог.
    """
    base_min = int(MIN_PROBABILITY)
    base_hybrid = int(HYBRID_MIN_PROB)

    # базовые полосы RSI (как у нас в debug логике)
    call_band = (45.0, 65.0)
    put_band  = (35.0, 55.0)

    if not (ADAPTIVE_PRO_MAX or QUALITY_MODE):
        return base_min, base_hybrid, call_band, put_band

    adj = 0

    # чем выше "пила" — тем строже
    if chop >= HYBRID_CHOP_THRESHOLD:
        adj += 6
        # в пиле держим RSI ближе к середине
        call_band = (48.0, 62.0)
        put_band  = (38.0, 52.0)

    # слабый импульс — повышаем порог
    if impulse < HYBRID_IMPULSE_CUTOFF:
        adj += 4

    # низкая волатильность часто даёт ложные 1м сигналы
    if atr_pct < (ATR_THRESHOLD * 0.85):
        adj += 3

    # сильный импульс + нормальная волатильность => можно чуть мягче
    if impulse >= (HYBRID_IMPULSE_CUTOFF + 5.0) and atr_pct >= (ATR_THRESHOLD * 1.2) and chop < HYBRID_CHOP_THRESHOLD:
        adj -= 2

    # QUALITY_MODE всегда делает строже минимум на +2
    if QUALITY_MODE:
        adj = max(adj, 2)

    min_prob = int(clamp(base_min + adj, base_min, 95))
    hybrid_min = int(clamp(base_hybrid + adj, base_hybrid, 95))
    return min_prob, hybrid_min, call_band, put_band
...
async def compute_signal(symbol: str) -> Optional[Signal]:
    ...
    impulse = rel  # atr% / thr
    dyn_min, dyn_hybrid_min, rsi_call_band, rsi_put_band = adaptive_thresholds(atrp, chop, impulse)
    min_prob = dyn_hybrid_min if HYBRID_MODE else dyn_min
    if probability < min_prob:
        return None

    # Доп. фильтр RSI (включается в QUALITY_MODE / ADAPTIVE_PRO_MAX)
    if QUALITY_MODE or ADAPTIVE_PRO_MAX:
        lo, hi = (rsi_call_band if direction == "CALL" else rsi_put_band)
        if not (lo <= rsi14 <= hi):
            return None
...
async def job_signals(context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        def _effective_prob(s: Signal) -> float:
            p = float(s.probability)
            if DIVERSIFY_WINDOW_MINUTES > 0:
                last = STATS.get('last_signal') or {}
                if last.get('symbol') == s.symbol and last.get('time'):
                    try:
                        t_last = datetime.fromisoformat(last['time'])
                        dtm = (now_tz() - t_last).total_seconds() / 60.0
                        if dtm < float(DIVERSIFY_WINDOW_MINUTES):
                            p -= float(DIVERSIFY_PENALTY_POINTS)
                    except Exception:
                        pass
            return p

        found.sort(key=_effective_prob, reverse=True)
