...
# - Не молчит (но не спамит)
# - Торговое время ПН–ПТ 10:00–20:00 (TZ)
...
# Сканер / интервалы
SIGNAL_INTERVAL_SECONDS = int(os.getenv("SIGNAL_INTERVAL_SECONDS", "600"))  # 10 минут по умолчанию (экономит API)
PULSE_INTERVAL_SECONDS = int(os.getenv("PULSE_INTERVAL_SECONDS", "600"))    # 10 минут
EXPIRY_MINUTES = int(os.getenv("EXPIRY_MINUTES", "3"))                      # 3 минуты по умолчанию
...
# Анти-спам и паузы
COOLDOWN_MINUTES = int(os.getenv("COOLDOWN_MINUTES", "6"))  # пауза после сигнала по той же паре
...
def ensure_day_reset() -> None:
    ...
    if STATS["day"] != today:
        ...
        # cooldowns оставляем — можно сбрасывать, но лучше оставить
...
def require_env() -> None:
    ...
    if OWNER_ID == 0:
        log.warning("OWNER_ID не задан (0). Кнопки WIN/LOSS будут доступны только формально, но owner-check не пройдёт.")
...
def td_quote_price(symbol: str) -> float:
    """Один запрос — цена для отчёта после экспирации."""
    url = f"{TD_BASE}/quote"
...
def atr_percent(df: pd.DataFrame, period: int = 14) -> float:
    ...
    if c == 0 or pd.isna(a) or pd.isna(c):
        return 0.0
    return float((a / c) * 100.0)  # в процентах
...
class Signal:
    ...
    entry_time: datetime
    exit_time: datetime
    signal_id: str  # внутренний id (для кнопок)
...
def per_symbol_atr_min(symbol: str) -> float:
    """
    Можно ужесточать отдельные пары.
    Сейчас: JPY пары часто "пилят" — ставим не ниже базового.
    """
    base = ATR_THRESHOLD * GLOBAL_ATR_MULT
    if "JPY" in symbol.upper():
        return max(base, 0.020)  # минимально разумный порог для JPY
    return base


def adaptive_atr_threshold(df: pd.DataFrame) -> float:
    """
    Адаптивный порог: берём медиану ATR% за последние ~120 свечей
    и ставим порог примерно 70% от неё (чтобы не пропускать "плоский" рынок).
    """
    # считаем ATR% по ряду
    a = atr(df, 14)
    ...
    if len(tail) < 30:
        return per_symbol_atr_min("X")  # fallback
    med = float(tail.median())
    return max(0.012, med * 0.70)  # не ниже 0.012%


def confirm_higher_trend(symbol: str) -> Optional[str]:
    """
    Старший тренд на TREND_TF: EMA50 vs EMA200.
...
def compute_signal(symbol: str) -> Optional[Signal]:
    ...
    # Жёсткий запрет совсем низкого ATR
    if atr_pct < 0.012:
        return None
    ...
    if TREND_FILTER:
        ...
        if ht != direction:
            return None
        score += 10  # бонус за совпадение

    # Волатильность как бонус
...
async def job_send_signals(context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    for sig in sigs:
        ...
        # Планируем авто-отчёт после экспирации (без ID в тексте)
        when = sig.exit_time
        ...
        if context.job_queue:
            context.job_queue.run_once(_expiry_job, when=when)
...
def main() -> None:
    ...
    log.info("%s | started | TZ=%s | symbols=%s | mode=%s top=%s",
             CHANNEL_NAME, TIMEZONE_NAME, SYMBOLS, SEND_MODE, TOP_N)
